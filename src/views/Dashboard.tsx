...
export const Dashboard: React.FC = () => {
  ...
  const [paginatedConsumibles, setPaginatedConsumibles] = useState<
    Consumible[]
  >([]);
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (user?.rol === "TECNICO") {
        ...
        // Filter assets
        const personaToEmpresaMap = new Map(
          personasList.map((p: any) => [p.id, p.empresa_id]),
        );
  ...
  const getActiveTechForSede = (empresaId: number, empresaNombre: string) => {
    ...
    const activeGuard = guardias.find(
      (g) =>
        g.fecha.split("T")[0] === todayStr &&
        (g.empresa_id === empresaId ||
          g.empresa_id === null ||
          g.empresa_id === undefined),
    );
    if (activeGuard) {
    ...
    const isSpecialSede = ["GAMETOWN", "TEATRO", "APPARCA"].some((name) =>
      empresaNombre.toUpperCase().includes(name),
    );
    ...
    if (assignedTechs.length === 0) {
      const anySedeTechs = technicians.filter((tech) =>
        tech.empresa_ids?.includes(empresaId),
      );
      if (anySedeTechs.length > 0) {
        return anySedeTechs
          .map((t) => t.nombre_completo.split(" ")[0])
          .join(", ");
      }
      return "TI General";
    }
  ...
  if (loading) {
    return (
      <div className="dashboard-loading animate-fade">
        ...
        <p className="text-muted">Analizando métricas del sistema TISMO...</p>
      </div>
    );
  }
    ...
    <div className="dashboard-container animate-fade">
      ...
      >
        <div className="welcome-left">
          ...
          >
            Aquí tienes el estado operativo de Shopping Managements Operadora
            para el día de hoy.
          </p>
        </div>
      </div>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
          ...
          >
            Próximas Guardias Programadas (Fines de Semana y Feriados)
          </span>
        </div>

        {guardias.filter(
          (g) => new Date(g.fecha) >= new Date(new Date().setHours(0, 0, 0, 0)),
        ).length === 0 ? (
          <p
            className="text-muted"
            style={{ fontSize: "11.5px", padding: "4px 0" }}
          >
            No hay guardias de fin de semana o feriados programadas.
          </p>
        ) : (
          <div
            style={{
      ...
      <div className="dashboard-grid">
        ...
        <div className="metric-card glass-panel glass-panel-interactive animate-slide-up">
          ...
          <div className="metric-footer">
            <span className="sub-metric">
              <strong className="color-critical">{criticalCount}</strong>{" "}
              Críticos
            </span>
            <span className="sub-metric">
              <strong>{altaCount}</strong> Alta
            </span>
          </div>
        </div>
        ...
        <div className="metric-card glass-panel glass-panel-interactive animate-slide-up">
          ...
          <div className="metric-footer">
            <span className="sub-metric">
              <strong>{resolvedTicketsCount}</strong> resueltos
            </span>
            <span className="sub-metric">
              <strong>{totalTicketsCount}</strong> totales
            </span>
          </div>
        </div>
        ...
        {(user?.rol === "ADMIN" || user?.has_inventory_access) && (
          <div className="metric-card glass-panel glass-panel-interactive animate-slide-up">
            ...
            <div className="metric-footer">
              <span className="sub-metric">
                <strong>{assignedAssets}</strong> Asignados
              </span>
              <span className="sub-metric">
                <strong>{stockAssets}</strong> Bodega
              </span>
            </div>
          </div>
        )}
        ...
        <div className="metric-card glass-panel glass-panel-interactive animate-slide-up">
          ...
          <div className="metric-footer">
            <span className="sub-metric">
              <strong>{finishedProjects.length}</strong> Finalizados
            </span>
            <span className="sub-metric">
              <strong>{proyectos.length}</strong> Totales
            </span>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div className="operation-column glass-panel">
          ...
          <div
            className="recent-tickets-list"
            style={{
              opacity: loadingTicketsPage ? 0.6 : 1,
            ...
            {paginatedTickets.length === 0 ? (
              <div className="empty-operation text-center py-4">
                <p className="text-muted mt-2">
                  ¡Todo al día! No hay tickets pendientes de soporte.
                </p>
              </div>
            ) : (
              paginatedTickets.map((ticket) => (
                <div key={ticket.id} className="mini-ticket-row">
                  <div className="mini-ticket-left">
                    <span
                      className={`mini-priority-dot priority-${ticket.prioridad.toLowerCase()}`}
                      title={ticket.prioridad}
                    ></span>
                    <div className="mini-ticket-info">
                      <span className="mini-ticket-title">{ticket.titulo}</span>
                      <span className="mini-ticket-desc text-muted">
                        {ticket.categoria} •{" "}
                        {ticket.persona_solicitante || "Sede"}
                      </span>
                    </div>
                  </div>
                  <div className="mini-ticket-right">
                    <span
                      className={`badge badge-${ticket.estado.toLowerCase().replace(" ", "")}`}
                    >
                      {ticket.estado}
                    </span>
                  </div>
          ...
          {ticketsTotal > 4 && (
            <div className="dashboard-pagination">
              ...
              <button
                ...
                onClick={() => handleFetchTicketsPage(ticketsPage + 1)}
                disabled={
                  ticketsPage === (Math.ceil(ticketsTotal / 4) || 1) ||
                  loadingTicketsPage
                }
                title="Siguiente"
              >
        ...
        {(user?.rol === "ADMIN" || user?.has_inventory_access) && (
          <div className="operation-column glass-panel">
            <div className="column-header">
              ...
              {consumiblesTotal > 0 && (
                <span className="warning-count-badge">{consumiblesTotal}</span>
              )}
            </div>
            <div
              className="low-stock-list"
              style={{
                opacity: loadingConsumiblesPage ? 0.6 : 1,
                transition: "opacity 0.2s",
              }}
            >
              {paginatedConsumibles.length === 0 ? (
                <div className="empty-operation text-center py-4">
                  <p className="text-muted mt-2">
                    Stock robusto. Todos los consumibles por encima del mínimo.
                  </p>
                </div>
              ) : (
                paginatedConsumibles.map((c) => (
                  <div key={c.id} className="low-stock-row animate-fade">
                    <div className="low-stock-info">
                      ...
                      <span className="consumable-stock text-muted">
                        Stock:{" "}
                        <strong className="color-critical">
                          {c.stock_actual}
                        </strong>{" "}
                        / Mínimo: {c.stock_minimo} {c.unidad_medida}
                      </span>
                    </div>
                    <div className="stock-progress-bar-container">
                      <div
                        ...
                        style={{
                          width: `${Math.max(5, Math.min(100, (c.stock_actual / c.stock_minimo) * 100))}%`,
                        }}
                      ></div>
                    </div>
                  </div>
            ...
            {consumiblesTotal > 4 && (
              <div className="dashboard-pagination">
                <button
                  className="dashboard-pagination-btn"
                  onClick={() =>
                    handleFetchConsumiblesPage(consumiblesPage - 1)
                  }
                ...
                <span className="dashboard-pagination-info">
                  Pág. {consumiblesPage} de{" "}
                  {Math.ceil(consumiblesTotal / 4) || 1}
                </span>
                <button
                  className="dashboard-pagination-btn"
                  onClick={() =>
