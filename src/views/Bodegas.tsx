...
export const Bodegas: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const [bodegasData, empresasData] = await Promise.all([
        inventoryService.getBodegas(),
  ...
  const filteredBodegas = bodegas.filter((b) => {
    const matchesSearch = b.nombre.toLowerCase().includes(search.toLowerCase()) || 
      ...
      (b.empresa_nombre && b.empresa_nombre.toLowerCase().includes(search.toLowerCase()));
    
    const matchesSede = selectedSedeFilter === 0 || b.empresa_id === selectedSedeFilter;
  ...
  return (
    <div className="view-container animate-fade">
      <div className="view-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          <h1 className="view-title">Bodegas de Inventario TI</h1>
          <p className="view-subtitle text-muted">Administra los espacios físicos de almacenamiento y stock de activos por sede</p>
        </div>
        <button className="btn btn-primary" onClick={openCreateModal} style={{ display: 'inline-flex', alignItems: 'center' }}>
      ...
      <div className="filters-card glass-panel" style={{ padding: '16px', marginBottom: '24px', display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: '180px' }}>
          <select
            ...
            onChange={(e) => setSelectedSedeFilter(Number(e.target.value))}
            style={{ width: '100%' }}
          >
