...
interface Usuario {
  ...
  recibir_notificaciones_correo?: number | boolean;
  nivel_soporte?: "N1" | "N2";
...
export const Usuarios: React.FC = () => {
  ...
  // Fetch usuarios when page or search changes
  useEffect(() => {
  ...
  const fetchUsuarios = async (
    ...
    try {
      console.log(
        `[Usuarios] Consultando usuarios - page: ${pageNumber}, search: "${searchVal}"`,
      );
  ...
  const openEditModal = (u: Usuario) => {
    ...
    setRecibirNotificacionesCorreo(
      u.recibir_notificaciones_correo === undefined
        ? true
        : Boolean(u.recibir_notificaciones_correo),
    );
    setGeneratedPassword(null);
    setError(null);
    setShowModal(true);
  ...
  const handleToggleNotificacionesCorreo = async (u: Usuario) => {
    ...
    try {
      ...
      setUsuarios((prev) =>
        prev.map((item) =>
          item.id === u.id
            ? { ...item, recibir_notificaciones_correo: nextVal }
            : item,
        ),
      );
    } catch (err: any) {
      showAlert(
        err.message || "Error al actualizar notificaciones por correo.",
      );
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isEditing && !password) {
      setError(
        "Debes ingresar o generar una contraseña para un nuevo usuario.",
      );
      return;
    }
  ...
  const handleDelete = async (id: number, name: string) => {
    if (
      !(await showConfirm(
        `¿Estás seguro de que deseas eliminar permanentemente la cuenta de "${name}"?`,
      ))
    ) {
      return;
    }
    ...
    <div className="inventario-view animate-fade">
      <div className="view-header">
        <div>
          ...
          <p className="text-muted">
            Administra los técnicos de planta, directores y administradores de
            la plataforma
          </p>
        </div>
      ...
      >
        ...
        <button
          ...
          onClick={() => fetchUsuarios(page, debouncedSearch)}
        >
          Actualizar
        </button>
      </div>
                ...
                filteredUsuarios.map((u) => (
                  <tr key={u.id} className="table-row-hover animate-slide-up">
                      ...
                      >
                        ...
                        {u.rol_nombre === "TECNICO" && (
                          <span
                            ...
                            style={{ fontSize: "11px" }}
                          >
                            Soporte {u.nivel_soporte || "N1"}{" "}
                            {u.grupo_n2 ? `(${u.grupo_n2})` : ""}
                          </span>
                        )}
                      </div>
                    </td>
                    ...
                    <td>
                      ...
                      >
                        {u.empresa_inventario_nombres &&
                        u.empresa_inventario_nombres.length > 0 ? (
                    ...
                    <td>
                      <span
                        className={`badge ${u.is_active ? "badge-done" : "badge-baja"}`}
                        style={{ fontSize: "9px" }}
                      >
                    ...
                    <td style={{ textAlign: "right" }}>
                      <button
                        className="btn btn-secondary"
                        style={{ padding: "5px 7px", marginRight: "6px" }}
          ...
          >
            ...
            >
              ...
              <div className="form-group">
                ...
                <div style={{ display: "flex", gap: "8px" }}>
                  <input
                    ...
                    placeholder={
                      isEditing
                        ? "Ingresa una nueva o genera..."
                        : "Ingresa contraseña o genera..."
                    }
                    value={password}
              ...
              <div className="form-group">
                ...
                >
                  <option value={1}>
                    ADMIN (Control total y reportes globales)
                  </option>
                  <option value={4}>
                    SUPERVISOR (Control total, reportes, turnos y nivel soporte)
                  </option>
                  <option value={2}>
                    TECNICO (Acceso a sus propios tickets y chats)
                  </option>
                  <option value={3}>
                    USUARIO (Acceso básico de solicitud y lectura)
                  </option>
                </select>
              </div>
              ...
              {(Number(rolId) === 2 || Number(rolId) === 4) && (
                <>
                  <div className="form-group">
                    ...
                    >
                      <option value="N1">
                        Nivel 1 (Helpdesk / Presencial en CC)
                      </option>
                      <option value="N2">
                        Nivel 2 (Especialista / Infraestructura y Desarrollo)
                      </option>
                    </select>
                  </div>
              ...
              <div className="form-group">
                <label className="form-label">
                  SEDES FÍSICAS DE SOPORTE (ASIGNACIÓN DE TICKETS)
                </label>
                <p
                  className="text-dim"
                  style={{
                ...
                >
                  Sedes bajo la guardia y asignación automática de tickets de
                  soporte técnico.
                </p>
                <div
                  style={{
              ...
              <div className="form-group">
                <label className="form-label">
                  SEDES FÍSICAS DE INVENTARIO (CONTROL DE ACTIVOS)
                </label>
                <p
                  className="text-dim"
                  style={{
                ...
                >
                  Sedes donde se permite el control de activos, consumibles y
                  bodegas físicas.
                </p>
                <div
                  style={{
              ...
              >
                <input
                  type="checkbox"
                  id="recibirNotifCorreo"
                  checked={recibirNotificacionesCorreo}
                  onChange={(e) =>
                    setRecibirNotificacionesCorreo(e.target.checked)
                  }
                  disabled={submitting}
                  style={{ width: "18px", height: "18px", cursor: "pointer" }}
                />
                <label
                  htmlFor="recibirNotifCorreo"
                  className="form-label"
                  style={{ margin: 0, cursor: "pointer" }}
                >
